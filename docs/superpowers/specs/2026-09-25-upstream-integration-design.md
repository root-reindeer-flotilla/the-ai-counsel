...
- **`parsed_ranking`** holds **global** labels: local label → model → the global label for that model. This keeps upstream's `calculate_aggregate_rankings(results, label_to_model)`, the heatmap, and the MCP output correct without any changes.
